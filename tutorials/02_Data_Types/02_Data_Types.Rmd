...
print(pi)
```
Integer (for loops; well talk later)
```{r, echo=T, eval=F}
class_number <- 215
```
Logical T/F
...
## Why do we care about types of data structures
Logical
```{r, echo=T, eval=F}
...
which_entries_match <- a==b #paste this into the sandbox and look at the result
```
...
```{r ex, exercise = TRUE, exercise.setup = "setup1"}
