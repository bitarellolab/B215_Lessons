...
**Ex.4)** Check whether `mass_metabolism` is a tibble. Try to predict this based on what you know about the way ypu've just read in the data. Hint: you can use the `is_tibble` function, you will need to install and load the `tibble` package first. Does your prediction match the result of `is_tibble`? How did you know? 
...
You may have noticed a message from R that asks you to adjust the bins. Feel free use the `bins = ` parameter in your geom and see how that impacts the display of your data, 
...
**Ex.14)** Make a plot of body mass vs. metabolic rate, with log10 scaled axes (this stretches the axis, but keeps the numbers on the original scale), and the point size set to 3. Hint: use `scale_y_continuous(trans='log10')` and `scale_x_continuous(trans='log10')`. (permalink back to Lab 3)
...
**Ex.17)** Using `group_by`, `summarise`, and `n()`, calculate the range of body mass for each family as well as the number of rows for each family. Save the output to an object called `body_mass_summary`. Why do you think some ranges are zero?
