...
In the space below, create an object called `which_entries_match` which contains the result of your asking if `a` is the same as `b`. In the second line, print the object your created remember from Lab 1 how we ask this in R. (Tip, it's not simply `a=b` because that will try to assign object `b` to object `a`.)
