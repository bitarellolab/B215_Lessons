...
There are a few functions in R we can use to test the data type of the `pi` variable we just made. `is.numeric()` and `is.integer()` return a simple TRUE or FALSE in answer to the prompt. `class()`, on the other hand, is more general and will provide which data type the variable or object is. Try these out here:
...
Sometimes you can convert one type into another. Let's use the example variable `class_number` I've created below. 

```{r, echo=T, eval=F}
class_number <- 215
```

There are a few functions in R which allow you to convert data. These functions will usually take the form as.(insert data type you want to convert to). For example to convert something into an integer you'd use the function `as.integer()` and to convert to numeric you'd use `as.numeric()`. Try this out below: run the `class()` function on `class_number` to determine if it is numeric or integer data, then use the appropriate function to convert it.
...
Try out `isTrue` and `isFalse` on a variable which I have made and provided in your environment, `true_or_false` :
...
Try `class()` and `is.character()` out on this string!:
...
Try converting `hominin_genus` into a factor, using the earlier `as.integer()` and `as.numeric()` examples as a model. Then, run `class()` on your converted variable to check your work and print it
...
```{r examplenum, exercise = TRUE, exercise.setup = "setupnum"}
      avg_growth <- #mean of growth_data
      med_growth <- #median of growth_data
                    #print avg_growth
                    #print med_growth
```

```{r examplenum-solution}
     avg_growth <- mean(growth_data) #mean of growth_data
     med_growth <- median(growth_data) #median of growth_data
     print(avg_growth) #print avg_growth
     print(med_growth) #print med_growth
...
print(sum(x)) #sum of all values in x
print(x>y)
```

Practice these a bit here, you might even try making your own new numeric or integer vectors:
...
max(x)  #largest number in x
min(x)  #smallest number in x
```
...
Besides using the `mean()` function you can also find the mean in R by hand. To do this, you might start by adding up your total number of cells. You could define a variable `total_cells` where you put in the result of running the function `sum` on the vector `cell_counts`. Do that and then print the `total_counts` object:
...
Let's return to character data with a more practical example. In bioinformatics, character strings are often used to represent DNA sequences.
...
This example obviously uses just a few data points, but when processing DNA data, biologists may deal with hundreds of sequencing fragments at a time. The volume of data necessitates the use of a computer. Here we will practice working with this character data. Suppose you want to look at the first sequence from a long vector. You can select an element from vectors (also called indexing) in R by using square brackets after the name of the vector, with the number inside the brackets indicating the which number value you are interested in. Paste the following code to define a character variable containing the first sequence in the vector and then print it. `first_sequence <- dna_sequences[1] # DNA sequence`:
...
In the space below, create an object called `which_entries_match` which contains the result of your asking if `a` is the same as `b`. In the second line, print the object you created. Recall from Lab 1 how to ask this in R. (Hint/lab 1 link)
...
R loves vectors and tends to treat everything like a vector: even an object with only one element is a vector. E.g., `a<-9` is simply a vector of length `1`.
...
Coercing from Boolean to numeric means we can easily count the number of TRUEs and FALSES by summing the dating.
...
```{r, echo=T, eval=F}
a<-c(0, -5) #numeric vector
b<-c("ACGT", "TTGC") #character vector
d<-c(-1, 15000) #numeric vector
f<-"AGCT" #character vector
g<-2L #integer vector
h<-FALSE #logical vector
mat1<-matrix(c(a, b)) #combining vectors a and b into a matrix
```

Now use the `class()` and `typeof()` functions, to confirm `mat1` is a matrix and determine the type of element the matrix contains. Try to predict what the result of `typeof()` will be before you run your code.

```{r sandbox13, exercise=TRUE, eval = FALSE}
 #use class() to confirm it's a matrix
 #use typeof() to see the type of element contains
```

As you can see, the numeric data from `a` was coerced into character data to match `b`.
...
matrix1 <- cbind(vector1, vector2) #vector1 becomes column 1, vector 2 becomes column 2

# Combine vectors as rows to create a matrix
matrix2 <- rbind(vector1, vector2) #vector 1 becomes row 1, vector 2 becomes row 2
...
Print matrix 4 and the result to see the before and after
...
The key difference between data frames and matrices is that in matrices different columns can have elements of different types.
...
To demonstrate the use of data frames for storing multifaceted information which may require multiple data types, let's return to our hominin example. Here, we are constructing a data frame which contains the genus names we saw earlier, and adding species names and hypothetical genome sizes for these early hominins (which we have no way of knowing yet).
...
In the following box, separate all the rows with "Australopithecus" in the `Genus` column using the function `filter` from the dplyr package, `dplyr` has a whole host of useful functions that make working with data frames convenient. We will learn a lot more about the `dplyr` package in future lessons. Filter is just the tip of the iceberg.

Follow the format: `filter(dataframe_name, columnname == "Australopithecus")` and save the results as `australopithecus_genome_length`. Then, print `australopithecus_genome_length`. Notice that the column name you are filtering from is not in quotes, but the row data you are filtering for is. This is because R is sorting through all the character data in your column of interest to pull out the ones which contain the word Australopithecus, so you refer to what you're filtering for in the character data format.  
...
Now, using the data frame you made (`australopithecus_genome_size`), use the function `summary` on the column of that data frame which contains the genome information to calculate how variable hominin genomes are. Save that result into the object `summary_results` and print out that object.
...
Print the result and use double square brackets -`[[]]` -to select the third item from the list. We use double brackets to index a list, similar to indexing vectors or data frames with single brackets.
...
An array in R is a multi-dimensional data structure for storing homogeneous data (i.e., data of the same type: numeric, logical, character, etc). In that way, it is similar to a matrix. It can be viewed as a multi-dimensional version of matrices. You can think of an array as a multi-dimensional table of data, with each dimension being a different attribute. Below, the attributes are Gene, Sample, and Trial.
...
Now we are going to practice working with an actual data set. We will be using one from the `datasets` package called `chickweights`. It's from an experiment on the effect of diet on early growth of chicks. If you want to learn more about the data set type ??chickweights into your console (later, not while you are taking this quiz!)
...
```{r chicks1, exercise = TRUE}
         ...
         datasets::ChickWeight   # create chicks object
                                 # use head on chicks
```
...
Lets see which diet worked best! This data shows us the same chicken at multiple times. If we only want to see the final outcome, we can filter for the final weight. Using the `dplyr` package's `filter` function to select only chicks with time 21 from the `chicks` object. Name this resulting data frame `FinalChicks`. Then print it.
...
learnr::quiz(caption="",
  question("Which of the following statements is correct?",
    ...
    answer("Column 2 of `result` will be: `TRUE, FALSE, FALSE, FALSE, FALSE, TRUE, FALSE, FALSE, FALSE, TRUE`"),
