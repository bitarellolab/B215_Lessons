...
Now lets go back to our temperature functions from earlier, having learned everything we need to make it two way. 
```{r}
CtoF<-function(c){ #Celsius to Fahrenheit
f<- (c *1.8) + 32
return(f)
}

FtoC<-function(f){ #Fahrenheit to Celsius
c<- (f-32)/1.8
return(c)
}
...
* If you need a refresher or are unfamiliar with the binomial test, these slides are a good explanation (link)


## Calculating binomial probabilities
Before we dive into the case study, let's use some examples to familiarize ourselves with how the binomial functions in R work.

### Example 1, by hand : finding the probability of 2 out of 3 children in a household being born in the same month.

We'll start by using R to calculate binomial probabilities without the help of a function
...
### Example 2 with dbinom(): what is the probability of six successes in 27 trials, where p of success is 0.25.
...
Now we will move on to a different binomial function- `binom.test` used for conducting a binomial test. Instead of giving the probability that a certain number of successes will occur, it gives the probability that the number of successes provided came from a binomial distribution. The output of the function is. p-value from which we either accept or reject the null hypothesis. 
...
* I rolled my personal super special die 80 times during a boardgame and got a 6 in 19 of those. My buddies accuse me and say my die is perhaps too good to be true. I plead innocence and boast about my cosmic luck! What do you think? (hint)
...
For more info or a refresher on Agresti-Coull and 95% CIs, check out this slideshow (link)
...
Run a two-sided binomial test to see if the sample proportion differs significantly from the population proportion. Remember to decide your alpha value before you run the test in order to make sure you're doing honest testing.
