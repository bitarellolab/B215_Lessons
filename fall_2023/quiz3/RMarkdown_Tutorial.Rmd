...
Now that we've gone over how to run our code chunk, let us turn to the output of running said chunk. In the example above, I created a vector which I named `a` and then printed it. Recall Lab 1 and instructions on the layout of R Studio. Because we're working in an RMD you can actually see the Console, Environment, and available files as you work. When you run the line creating the vector, it copies the code down into the console and adds the variable `a` into your environment. When you run the line which prints `a` the vector appears in the RMD, in a box just below the chunk itself. Certain functions (especially in dplyr, which we will get to in the rest of Lab 3 today), display results directly in the file without the use of `print()`. This output is relevant because the aforementioned chunk customization has to do with 
