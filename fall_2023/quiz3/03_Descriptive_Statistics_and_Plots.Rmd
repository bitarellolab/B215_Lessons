...
1. Take the tutorial which is located in the files for this project and named RMarkdown_Tutorial.
...
A (somewhat annoying) quirk of R is that if we try to take the mean of a list of numbers that include missing data, we get an NA for the result! We began to cover dealing with missing data in Lab 2, while looking at the variable age, let's return to `na.rm =` in greater depth. 
...
Even more metrics include range and interquartile range which are the difference between the max and the min points in the data and the third quartile and first quartile in the data respectively. You can call these functions as range() and IQR() in the exact same format as mean() and sd().
...
*in general however, you should still use tidy format for naming vraiables, it makes working with the tibble after it is imported easier, and gtes you in the habit of naming this correctly for coding
...
Let's use the `iris` data set from the `datasets` package. This data set contains anatomical measurements of three species of iris. Specifically, the data lists the petal and sepal lengths and widths in centimeters.
...
`glimpse()` is different than `head()` in that it gives the dimensions of the data, and as much data as is in the columns that it can fit.
...
In R, the order of operations doesn't work like English. Be sure to write out a _complete_ test on each side of a boolean operator. In the example above, this is pretty obvious as Petal.Width and Species come from different columns, but even if you wanted to filter for multiple Species, say virginica and setosa, you would need two complete logical statements separated by an `&` or a `,`. You can't write `filter(iris, Species == "setosa" | "virginica")`, even though you might say "finds all measurements from I. setosa or I. virginica".
...
We saw this in Lab1, so if you need to recap that, it might be a good idea... (link)
...
Note that we wrote `Species == "virginica"` rather than `Species = "virginica"`, this is because of the aforementioned logical comparison operators that `filter` employs. Keeping this in mind will help you iterate on the uses of filter you're comfortable with and will help you to differentiate its syntax from other dplyr functions which accomplish similar things (i.e. select). Comparison operators are key to using `filter()`. Let's look at an example using the OR operator.
Try filtering the rows of `iris` that have either "virginica" OR "setosa" in the `Species` column:

```{r filter3,  exercise=TRUE, exercise.setup="iris-setup"}
                    #filter for virginica or setosa
```
...
```{r filter4,  exercise=TRUE, exercise.setup="iris-setup"}
                    #use `table()` on the Species column
```

Here are two tips to help you use logical tests and Boolean operators in R:
...
You can also use the `!=` aka the "is not equal to" function to filter for all the rows in data set without a specific value. This can be useful for when you move on to exploratory data analysis with real world data.

Another useful dplyr filtering helper is `between()`. Run the code below to see its use. What does it do?

```{r filter6, exercise=TRUE}
between(iris$Sepal.Length,5,6)
```
If you add the command `which`, you get the index of the positions in the vector that fulfill the requirement of being between 5 and 6.
...
You can select a subset of variables by name with the `select()` function in `dplyr`. In more practical terms, `select()` pulls columns out of the data frame, while `filter()` pulls out rows. Run the code below to see the narrow data set that `select()` creates.
...
A data set often contains information that you can use to compute new variables. `mutate()` helps you compute those variables. Since `mutate()` always adds new columns to the end of a data set, we'll start by creating a narrow data set which will let us see the new variables.
...
The main difference between `summarise()` and `mutate()` is the type of function that you use to generate the new columns. `mutate()` takes functions that return an entire vector of output which is appended to the original data frame (adding a new column to the end). `summarise()` takes functions that return a single value (or summary). These values will appear in a new data frame that has as many rows as functions you perform on the original data. Unlike `mutate()` summarise will not add onto the old data set but create its own new data set with only the manipulated data you created within the function.


### `group_by()`

`summarise()` is even more useful when you pair it with `group_by()`. `group_by()` changes the unit of analysis of the data frame: it assigns observations in the data frame to separate groups, and it instructs dplyr to apply functions separately to each group. `group_by()` assigns groups by grouping together observations that have the same combinations of values for the variables that you pass to `group_by()`.
...
Another super useful function to use alongside `summarise()` and `group_by()` is `n()`. `n()` counts the number of rows, you can use it to create a column in a summarized data frame that lists the number of data points in each group. 

Example: We can make a count of how many people departed from each place in our titanic data:

```{r n(), exercise = TRUE, exercise.setup="setupnum"}
summarise(group_by(titanicdata, embarked), count = n())
...
In the previous section we learned how to do a bunch of things to data with tidyverse. 

each of these manipulations were completed in isolation and if we wanted to save our changes to perform further manipulations we would have to create a bunch of new variables to save those changes in or nest functions in a complex manner. In this section, we will learn a better way, combining operations with the pipe `%>%` operator.
...
Faceting allows us to use the concept of small multiples [@tufte1983] to highlight patterns. In other words, faceting will take your plot and make smaller connected subplots, divided by a category. It works in a similar way to `fill` or `color` but completely separates the data rather than just making the categories distinct from one another on a single plot.
...
```
If there were even more data points than shown here, using `alpha` as an argument in the geom to make the points a bit transparent could also help legibility 
...
The titanic data set does not have two numerical variables, so let’s use a different data set— This comes from a paper by Kamilar, J. M., & Cooper, N. from 2013, entitled "Phylogenetic signal in primate behaviour, ecology and life history". It uses a few different traits such as brain size and body mass to construct a phylogenetic tree for different primates. We'll make a scatter plot with the variables between the mean female brain size and the mean female body mass for each of the species.

```{r, eval=F}
primates <-read.csv("Kamilar_and_Cooper_supp2(Sheet1).csv")
```

To make a scatter plot of the variables Body.mass.female.mean and Brain.size.female.mean with ggplot, you need to specify the x and y variables, and use `geom_point()`:

```{r, eval=F}
ggplot(primates,   
  aes(x = Body.mass.female.mean,
  y = Brain.size.female.mean)) +
  geom_point()
```

Notice that this plot has quite a bit of data clustered in the bottom left, if we wanted to see the data more spread out, we could use a log transformation. In general, log transformation can be used to mitigate a skew in the data, making it easier to visualize without destroying the initial relationship demonstrated. This stretches the axis, but keeps the numbers on the original scale. Let's give this a try with our primates plot. In order to accomplish a log transformation in ggplot, we can use the arguments `scale_y_continuous(trans='log10')` and `scale_x_continuous(trans='log10')`. 
...
ggplot(primates,   
  ...
  y = Brain.size.female.mean)) +
    geom_point() +
    xlab("Mean Female Body Size") + 
    ylab("Mean Female Brain Size")
```
...
```{r primates1,exercise=TRUE, exercise.setup="setupnum"}
ggplot(primates,   
  aes(x = Body.mass.female.mean,
  y = Brain.size.female.mean)) +
    geom_point() +
...
Now that we've tested our tidyverse knowledge, let's return to the titanic data to test out our skills with piping and using tidyverse functions in conjunction with one another for more complex analysis. The titanic data has age, sex, class of the passenger and whether or not they survived. The summarize function is super useful here for analyzing the different factors that are correlated with a passenger's chance of survival. In this example, we will explore age and survival rate. Try using `summarise()` and `group_by` to make a table showing the mean age of those who survived versus those who did not. Print the new data frame to see.


```{r examplenum7, exercise = TRUE, exercise.setup = "setupnum"}
 mean_ages <- titanicdata2 %>% #creating variable mean_ages
                               #group by survival
                               #summarize for mean age
                               #print
...
Let's return to the iris data set for our next example. We have the length and width of the petals, but let's say we wanted to add a row with a (very) rough approximation of the petal area for each of the iris species. Use the mutate, summarise, and group_by functions to accomplish this. 
...
### Analyzing the data
Lets see which diet worked best! This data shows us the same chicken at multiple times. If we only want to see the final outcome, we can filter for the final weight. Using the dplyr filter function to select only chicks with time 21. Then, using the pipe operator instead of saving a new version of the dataframe, group the chicks by diet and find the mean weight at time 21 for the chicks on each of the four diets. Name this resulting data frame avg_chick_wgts. Then print it. 
...
Congrats! You've now had an intro to the tidyverse, plotting, and data analysis. Next week, we'll take these skills to the next level with actual Exploratory Data Analysis, but for now, be proud of how much you've learned in just three weeks!
