...
Well, these are fundamental concepts in programming, and they transcend R -- they apply to any programming language.
...
Factors are great for storing **categorical variables**. E.g. Champions of the American Major League Soccer (or, for the rest of the world, football) in the past 10 years.(Note, the MLS also includes Canadian and Mexican teams.)
...
You will notice that printing `mls_champions_factor` will spit out the vector, followed by `8 Levels: Atlanta United FC Columbus Crew Los Angeles FC Lost Angeles Galaxy New York City FC ... Toronto FC`. 

What this means is that the function `as.factor` converted our simple character vector into a data type that is better equipped at handling categorical variables. The "levels" are the different categories observed in the data. Same of the advantages of this will become clearer later on (next week) when we start plotting things and summarizing them.
...
On the other hand, there are things that only make sense for characters. For example, the function.
...
Suppose you are studying the growth rate of a bacterial culture. You have the sizes of the bacterial colonies at different time points and you want to calculate the average growth rate for different strains of bacteria in the same type of medium.
...
```
Suppose you want to take the mean and median of these values. Paste `avg_growth <- mean(growth_data)` and `med_growth <- median(growth_data)` and then print `avg_growth` and `med_growth`:
...
Suppose you are studying the growth rate of a fungal culture. With the microscope, you are able to actually count them over time. You have the sizes of the bacterial colonies at different time points and you want to calculate the average number of fungal colonies you find in your culture over the course of seven days.
...
To get the mean, you might want to add up your total number of cells. You could define a variable `total_cells` where you put in the result of the function `sum` on the vector `cell_counts`. Do that and then print the `total_counts` object:
...
```{r, echo=T, eval=F}
cell_counts <- c(100, 200, 150, 175, 225,  122, 270) 
cell_counts_withNA<-c(100, 200, 150, 175, NA, 122, 270)
total_cells <- sum(cell_counts)
...
total_cells_withNA/length(cell_counts_withNA)
total_cells_withNA_correct/length(cell_counts_withNA)
```
...
Suppose you want to look at the first sequence from a long list. Paste the following code to define a character variable and then print it. `first_sequence <- dna_sequences[1] # DNA sequence`:
...
```
```{r examplechrindex, exercise = TRUE, exercise.setup = "setupchr"}
...
```{r sandbox9, exercise=TRUE, eval = FALSE}
...
g<-2L #integer
h<-FALSE #logical
a_and_d<-c(a,d)  #no problems
b_and_f<-c(b, f) #no problems
a_and_g<-c(a,g)  #oops?
a_and_b<-c(a,b) #oops
d_and_f<-c(d,f) #oops
d_and_g<-c(d, g) #oops?
...
Try these for yourself before you move on. You can also try other combinations. Check the class of each of the objects created by concatenation.
...
```{r dfex3-solution}
summary_results <-summary(australopithecus_genome_length$Genome.length)
...
Is the species endangered? Use list indexing (with `$`) to retrieve the information.
...
It's long, so we don't want to print the whole thing. Use a specific function called `head` to call just the first 6 rows of the object.
...
### Analyzing the data
Lets see which diet worked best! This data shows us the same chicken at multiple times. If we only want to see the final outcome, we can filter for the final weight. Using the `dplyr` package's `filter` function to select only chicks with time 21 from the `chicks` object.
...
#this chunk is giving me problems
#learnr::quiz(
