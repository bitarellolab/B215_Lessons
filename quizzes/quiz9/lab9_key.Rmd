...
Today will be less about complicated coding and more about statistics. We will
learn how to run:
...
* create a vector containing the expected proportions of each color of M&Ms according to the null hypothesis, in the following order: blue, brown, green orange, red, yello
...
* multiply your smaple size by the expected proportions to find the expected absolute values for each color
...
expected_counts<-n_total * expected_proportions
expected_counts
```
...
```{r ex1.6}
#your code here
my_chisq_test<-chisq.test(MMtable, p = expected_proportions)
my_chisq_test
```
...
```{r ex1.7}
#step 1
step1<-MMtable-expected_counts
step1
#step 2
step2<-step1^2
step2
#step 3
step3<-step2/expected_counts
step3
...
* define the degrees of freedom. Remember that for goodness of fit tests this is nr of categories - 0 - 1. Use `length()` to find the number of categories in MMtable instead of manually typing that in. Save the degrees of freedom as `my_df`.

* run the `pchisq` function and save the output as `my_pchisq`. Make sure you change the argument `lower.tail` to `FALSE`. Then print `my_pchisq`.


```{r ex1.8}
#calculate df
my_df<-length(MMtable)-1

#run pchisq using my_df and step4 as arguments. Set lower.tail to FALSE
my_pchisq<-pchisq(q=step4, df=my_df, lower.tail=F)
my_pchisq
```
...
table_extinctions<-table(number_of_extinctions)
table_extinctions
```
...
nrow(extinctData)
# which frequency is most common across the time blocks?
sort(table_extinctions)[length(table_extinctions)]
#how many time periods had 2 or less extinctions? 
length(number_of_extinctions[which(number_of_extinctions<=2)])
```
...
We’ll need the probabilities for all possible values of x from 0 to 20 (because this was the largest value observed in the data). `dpois` rquires two arguments: a vector with the number of successes (0,1, and so on) and the lambda value, which we calculated above (it's the mean).
...
expected_extinction_probabilities<-dpois(x=vals, lambda=mean_extinctions)
expected_extinction_probabilities
...
expected_extinction_counts<-length(number_of_extinctions)* expected_extinction_probabilities
#print expected_extinction_counts
expected_extinction_counts
...
chisq.test(x=observed_combined, p=expected_combined, rescale.p=TRUE)$statistic
```
...
```{r ex1.18}
#code here
chisq.test(x=observed_combined, p=expected_combined, rescale.p=TRUE)
```
...
Here, we will learn how to look at, and test for, the association between two categorical variables. We’ll use the Titanic data set as an example, looking at the association between the sex of passengers and whether they survived the accident. Let’s read the data from the .csv file:
...
```{r ex2.8}
#make cancerTable2
cancerTable2<-t(cancerTable)
#run riskratio
RR <- riskratio(t(cancerTable2), rev = "both", method = "wald")
RR
...
#check for NAs
table(is.na(sars$Ribavirin))
table(is.na(sars$Survival))

#Answers: Received ribavirin: 87, did not receive ribavirin: 132. 27 died, 202 survived. No NAs
```
...
sars_table<-table(sars$Ribavirin, sars$Survival)
sars_table
```
...
sars_prop_table<-sars_table/sum(sars_table)
...
#make new mosaic plot with different colors and axis labels
mosaicplot(sars_table, color = c("darkred", "gold"), xlab ="Treatment Group", ylab = "Survival")
...
```{r ex3.6}
chisq.test(sars_table, p=sars_prop_table, correct=F)
...
fisher.test(sars_table)$estimate

#obtain the 95% CI of the OR
fisher.test(sars_table)$conf.int
