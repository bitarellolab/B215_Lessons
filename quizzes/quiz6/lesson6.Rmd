...
2.1. First filter it to only keep the size and name column. 
```{r}

```

2.2. create a second data frame with summary statistics mean, median and SD of length. (what is interesting about this data??)
```{r}

```

2.3. Now let's take some samples. Take 3 samples of gene size with replacement on. Use n = 10, 100, 1000. Calculate the mean of these samples, and save each mean as mean10, mean100, and mean1000. 
```{r}

```

2.4. Compare these means to the population mean. Which is closest?
```{r}
