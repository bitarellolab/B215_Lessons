...
Notes for lesson^[Developed by Daphne Hansell ('24) and Dr. Bitarello]:


* [case study using the binomial distribution](#Part 1: Case study: Hospital readmission rates of acute ischemic stroke in California) 

* review loops  - for and while^[Previous two labs.] - before getting started

* [Part 2: functions](#Functions)
...
$$P[X]=\binom{n}{X}p^X(1-p)^{n-X}$$^[Note: we write these equations using a typesetting language called Latex. It's great and awesome but you don't need to know it, but now you know what this is.]
...
# now test that running FtoC() on the temperatures you obtained with CtoF gives the expected outputs:
...
Try using the logic of the function above to write a function called `classifyNumber` that uses an input number and returns a statatement (printed message) telling you whether: it's positive, negative or 0. 
Hint: You can copy and paste that code and just edit it.
...
**Ex.6:** So far we've been providing single numbers to our aguments, but any function that works for one number should also work for a vector of numbers.
...
Be careful, though, when you don't spell out the arguments as we did above: `dumny_function(2,3)` instead of `dummy_function(x = 2, power = 3)`. In those cases, R assumes you are passing arguments in the order in which they are defined in the function. 
...
* we will use the `is.null()` function to check if both parameters are NULL. We will use \&
to combine both statements since we are checking if both are NULL, not just one of them
...
calculate_area<-function(width = NULL, length = NULL){
        ...
        if(is.null(width) & is.null(length)){
                stop("Both width and length are undefined. Seriously, what do you hope will happen here sweety?")
        }
...
```{r ex10}
#first name your function "calculate_area" and add two arguments: length and widtg. 
